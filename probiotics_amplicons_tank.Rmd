---
title: "probiotics_amplicons_Tank"
author: "JM"
date: "12/2/2020"
...
#### 1st sequencing run
path <- "~/Desktop/Probiotics_Amplicons_Tank/cutadapt_NS1968R_Tank" ##update path as needed
list.files(path)
...
saveRDS(seqtab, "~/Desktop/Probiotics_Amplicons_Tank/cutadapt_NS1968R_Tank/seqtab.rds") ##update path as needed
```
...
#### 2nd sequencing run
path <- "~/Desktop/Probiotics_Amplicons_Tank/cutadapt_NS1995_Tank" ##update path as needed
list.files(path)
fnFs <- sort(list.files(path, pattern="_R1_cut.fastq.gz", full.names = TRUE))
fnRs <- sort(list.files(path, pattern="_R2_cut.fastq.gz", full.names = TRUE))
sample.names <- sapply(strsplit(basename(fnFs), "_"), `[`, 1)
...
saveRDS(seqtab, "~/Desktop/Probiotics_Amplicons_Tank/cutadapt_NS1995_Tank/seqtab.rds") ##update path as needed
```
...
#### 3rd sequencing run
path <- "~/Desktop/Probiotics_Amplicons_Tank/cutadapt_NS2051_Tank" ##update path as needed
list.files(path)
fnFs <- sort(list.files(path, pattern="_R1_cut.fastq.gz", full.names = TRUE))
fnRs <- sort(list.files(path, pattern="_R2_cut.fastq.gz", full.names = TRUE))
sample.names <- sapply(strsplit(basename(fnFs), "_"), `[`, 1)
...
saveRDS(seqtab, "~/Desktop/Probiotics_Amplicons_Tank/cutadapt_NS2051_Tank/seqtab.rds") ##update path as needed
```
...
```{r}
st1 <- readRDS("~/Desktop/Probiotics_Amplicons_Tank/cutadapt_NS1968R_Tank/seqtab.rds") ##update path as needed
st2 <- readRDS("~/Desktop/Probiotics_Amplicons_Tank/cutadapt_NS2051_Tank/seqtab.rds") ##update path as needed
st3 <- readRDS("~/Desktop/Probiotics_Amplicons_Tank/cutadapt_NS2051_Tank/seqtab.rds") ##update path as needed
...
saveRDS(seqtab.nochim, file="~/Desktop/Probiotics_Amplicons_Tank/tank.rds") ##update path as needed
# RELOAD THE SAVED INFO FROM HERE (if you have closed the project):
#seqtab.nochim <- readRDS("~/Desktop/Probiotics_Amplicons_Tank/tank.rds") ##update path as needed
...
```{r}
taxa <- assignTaxonomy(seqtab.nochim, "~/Desktop/Probiotics_Amplicons_Tank/silva_nr_v132_train_set.fa.gz", multithread=TRUE) ##update path as needed
...
ntaxa(ps10) #9
get_taxa_unique(ps, "Genus") #249
get_taxa_unique(ps5, "Genus") #17
get_taxa_unique(ps10, "Genus") #7
# use unfiltered dataset
...
get_taxa_unique(ps_ra, "Order") #112
get_taxa_unique(ps_ra, "Class") #53
n <- 53
...
# Check out vibrios too
vibrio<-subset_taxa(ps_ra, Genus=="Vibrio")
vibrio
